...
- 135 Vitest tests (systems, monetization, UI, time, economy, content, engine, e2e journey, Android config)
- Lightweight E2E: `tests/e2e/coreJourney.test.ts` (tap→buy→flush→restore + Daily Dump idempotency)
- Local: format, lint, typecheck, test, build, Android app + test APK assembly green
- Emulator: MainActivity + Capacitor bridge/WebView instrumented smoke green
...
- **Main validation GREEN** — https://github.com/SundreamSoftware/PoopClicker/actions/runs/31256380011
- **Android emulator smoke GREEN** — https://github.com/SundreamSoftware/PoopClicker/actions/runs/31256380065
...
| Unit/integration tests  | Verified (135 pass)                                                                                                 |
| Android `assembleDebug` | Verified locally (BUILD SUCCESSFUL)                                                                                 |
| Android emulator launch | **PASS** — MainActivity, Capacitor bridge, visible WebView                                                          |
...
| F26 | P2       | ANDROID      | Physical device run not executed; emulator smoke is green                       | No physical device in agent                               | Install v1.0.1 APK on device | adb           | OPEN     |
...
- v1.0.0 startup crash: missing Google Mobile Ads application-id metadata caused process termination before MainActivity. Fixed in v1.0.1; see `docs/ANDROID_STARTUP_INCIDENT.md`.
...
4. Install v1.0.1 on physical Android hardware and run the device matrix.
5. Generate a signed AAB after release secrets are configured.
