...
function trimTargetPool(targets: FloatingTarget[], max: number, now: number): FloatingTarget[] {
  ...
  const caught = targets.filter((t) => t.caught)
  return [...caught, ...live.slice(0, max)]
}
...
export function createEventRuntime(
  ...
  if (!def) return null
  if (flushCount < def.minFlushCount) return null
  ...
  switch (def.type) {
    ...
    case 'mega_clog': {
      ...
      const phaseTapTarget = Math.ceil(total / 3)
      return { ...base, phase: 1, phaseTapTarget, tapTarget: total }
    }
...
export function tickEventRuntime(
  ...
  if (next.type === 'golden_rain') {
    const live = next.targets.filter((t) => !t.caught && t.expiresAt > now)
    const spawnCount = Math.max(0, 4 - live.length)
    next.targets = [
      ...live,
      ...Array.from({ length: spawnCount }, (_, i) =>
  ...
  if (next.type === 'plumber_inspection' && !next.completed && !next.failed) {
    ...
    next = {
      ...
      inBandMs,
      lastCpsSampleAt: now,
      bandScore: Math.min(1, inBandMs / elapsed),
    }
  }
...
export function catchTarget(
  ...
  return {
    runtime: { ...runtime, targets, caughtCount, taps, completed },
    caught: true,
  }
}
...
export function evaluateEventCompletion(
  ...
  if (runtime.rewardClaimed) {
    return { completed: true, failed: false, awaitingChoice: false }
  }
  ...
  return {
    completed: false,
    failed: failTypes.includes(runtime.type),
    awaitingChoice: false,
  }
}
...
export function computeEventRewards(
  ...
  const pp = production.pps.mul(def.rewardPpMinutes * 60).mul(mult)
  const gtp = Math.floor(def.rewardGtp * mult)
  return { pp, gtp }
}
